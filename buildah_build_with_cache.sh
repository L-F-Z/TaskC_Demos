...
# 构建时间和镜像大小记录文件  
log_file="buildah_build_with_cache.log"  
...
for project in "${projects[@]}"; do  
    ...
    while [ $attempt -le $max_attempts ]; do  
        start_time=$(date +%s)  
        buildah bud -t ${project,,}-cpu -f cpuDockerfile .  
        build_status=$?  
        end_time=$(date +%s)  
        ...
        if [ $build_status -eq 0 ]; then  
            ...
            echo "${project}-cpu 构建时间：${build_time}秒，镜像大小：${image_size}" | tee -a "../$log_file"  
            # 将镜像保存到/tmp目录下  
    ...
    while [ $attempt -le $max_attempts ]; do  
        start_time=$(date +%s)  
        buildah bud -t ${project,,}-gpu -f gpuDockerfile .  
        build_status=$?  
        end_time=$(date +%s)  
        build_time=$((end_time - start_time))  
        ...
        if [ $build_status -eq 0 ]; then  
            ...
            echo "${project}-gpu 构建时间：${build_time}秒，镜像大小：${image_size}" | tee -a "../$log_file"  
            # 将镜像保存到/tmp目录下  
