...
usage() {  
    ...
    echo "  proj: 构建指定的项目，例如 ${BLUE}bash buildDocker_withCache.sh CLIP${NC}"  
    echo "  all : 构建所有项目，例如 ${BLUE}bash buildDocker_withCache.sh all${NC}"  
    echo "  支持同时构建多个项目，例如 ${BLUE}bash buildDocker_withCache.sh CLIP YOLOv5${NC}"  
