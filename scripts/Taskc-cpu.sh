...
usage() {  
    ...
    echo "  proj: 构建指定的项目，例如 ${BLUE}bash Taskc-cpu.sh CLIP${NC}"  
    echo "  all : 构建所有项目，例如 ${BLUE}bash Taskc-cpu.sh all${NC}"  
    echo "  支持同时构建多个项目，例如 ${BLUE}bash Taskc-cpu.sh CLIP YOLOv5${NC}"
    echo "Flags:"
    echo "  --no-cache   每次构建时清除缓存，例如 ${BLUE}bash Taskc-cpu.sh --no-cache all${NC}"  
    echo "  --full       使用full版blueprint，full优先级高于cpu/gpu，例如 ${BLUE}bash Taskc-cpu.sh --full all${NC}"
    echo "  --cpu        仅构建 CPU 版本的镜像，例如 ${BLUE}bash Taskc-cpu.sh --cpu all${NC}"  
    echo "  --gpu        仅构建 GPU 版本的镜像，例如 ${BLUE}bash Taskc-cpu.sh --gpu all${NC}"
