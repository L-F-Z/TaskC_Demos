...
usage() {  
    ...
    echo "Flags:"
    echo "  --no-cache   每次构建时清除缓存，例如 ${BLUE}bash Docker.sh --no-cache  CLIP${NC}"  
    echo "  --cpu        仅构建 CPU 版本的镜像，例如 ${BLUE}bash Docker.sh --cpu  CLIP${NC}"  
    echo "  --gpu        仅构建 GPU 版本的镜像，例如 ${BLUE}bash Docker.sh --gpu  CLIP${NC}" 
