...
usage() {  
    ...
    echo "  proj: 构建指定的项目，例如 ${BLUE}bash Apptainer.sh CLIP${NC}"  
    echo "  all : 构建所有项目，例如 ${BLUE}bash Apptainer.sh all${NC}"  
    echo "  支持同时构建多个项目，例如 ${BLUE}bash Apptainer.sh CLIP YOLOv5${NC}"
    echo "Flags:"
...
build_image2() {
    ...
    local output_sif="/tmp/${project,,}-${variant}.sif"
    local project_dir="${project_base_dir}/${project}"  
...
buildImage2() {
    ...
    if [ "$cpu_only" = true ]; then  
        ...
        curl http://192.168.143.41:9081/repository/storage/apptainer/${project}-cpu -o /tmp/${project}-cpu
    ...
    if [ "$gpu_only" = true ]; then  
        TIMESTAMP=$(date +%s) 
        echo "start, $TIMESTAMP" | tee -a "$log_file"
        curl http://192.168.143.41:9081/repository/storage/apptainer/${project}-gpu -o /tmp/${project}-gpu
...
buildImage() {
    ...
    TIMESTAMP=$(date +%s) 
    echo "start, $TIMESTAMP" | tee -a "$log_file"
    curl http://192.168.143.41:9081/repository/storage/apptainer/${project} -o /tmp/${project}
