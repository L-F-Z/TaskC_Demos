...
for project in "${projects[@]}"; do
    ...
    while [ $attempt -le $max_attempts ]; do
        start_time=$(date +%s)
        apptainer build /tmp/${project,,}-cpu.sif cpuApptainer.def
        build_status=$?
        end_time=$(date +%s)
        ...
        if [ $build_status -eq 0 ]; then
            ...
            echo "${project}-cpu 构建时间：${build_time}秒，镜像大小：${image_size} MB" | tee -a "../$log_file"
            break
        else
    ...
    # 构建GPU Apptainer镜像并记录时间和镜像大小
    echo "构建GPU镜像..."
    attempt=1

    while [ $attempt -le $max_attempts ]; do
        start_time=$(date +%s)
        apptainer build /tmp/${project,,}-gpu.sif gpuApptainer.def
        build_status=$?
        end_time=$(date +%s)
        build_time=$((end_time - start_time))
        ...
        if [ $build_status -eq 0 ]; then
            ...
            echo "${project}-gpu 构建时间：${build_time}秒，镜像大小：${image_size} MB" | tee -a "../$log_file"
            break
        else
